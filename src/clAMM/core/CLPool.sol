...
contract CLPool is ICLPool {
    ...
    modifier onlyGaugeFactory() {
        (, address gaugeFactory) = IFactoryRegistry(factoryRegistry)
            .factoriesToPoolFactory(address(factory));
    ...
    function mint(
        ...
        ICLMintCallback(msg.sender).uniswapV3MintCallback(
            amount0,
            amount1,
            data
        );
    ...
    function stake(
        ...
        if (tick >= tickLower && tick < tickUpper) {
            ...
            stakedLiquidity = LiquidityMath.addDelta(
                stakedLiquidity,
        ...
        if (positionUpdate) {
            Position.Info storage nftPosition = positions.get(
                nft,
                tickLower,
                tickUpper
            );
            Position.Info storage gaugePosition = positions.get(
                gauge,
                tickLower,
                tickUpper
            );
    ...
    function swap(
        ...
        while (
            ...
            if (state.sqrtPriceX96 == step.sqrtPriceNextX96) {
                ...
                if (step.initialized) {
                    ...
                    state.stakedLiquidity = LiquidityMath.addDelta(
                        state.stakedLiquidity,
                        nets.stakedLiquidityNet
                    );
                }
        ...
        if (cache.stakedLiquidityStart != state.stakedLiquidity)
            stakedLiquidity = state.stakedLiquidity;
        ...
        if (zeroForOne) {
            ...
            ICLSwapCallback(msg.sender).uniswapV3SwapCallback(
                amount0,
                amount1,
                data
            );
    ...
    ) external view override returns (uint256 rewardGrowthInside) {
        checkTicks(tickLower, tickUpper);
        if (_rewardGrowthGlobalX128 == 0)
            _rewardGrowthGlobalX128 = rewardGrowthGlobalX128;

        return
            ticks.getRewardGrowthInside(
                tickLower,
                tickUpper,
                slot0.tick,
    ...
    {
        stakedFeeAmount = FullMath.mulDivRoundingUp(
            feeAmount,
            _stakedLiquidity,
            _liquidity
        );
        (unstakedFeeAmount, stakedFeeAmount) = applyUnstakedFees(
    ...
    function applyUnstakedFees(
        ...
        returns (uint256 unstakedFeeAmount, uint256 stakedFeeAmount)
    {
        uint256 _stakedFee = FullMath.mulDivRoundingUp(
            _unstakedFeeAmount,
            unstakedFee(),
            1_000_000
        );
        ...
        returns (uint256 feeGrowthGlobalX128, uint256 stakedFeeAmount)
    {
        ...
        // if there is only unstaked liquidity
            ...
            (
                uint256 unstakedFeeAmount,
                uint256 _stakedFeeAmount
            ) = applyUnstakedFees(feeAmount, 0);
        ...
        // if there are staked and unstaked liquidities
            ...
            (uint256 unstakedFeeAmount, uint256 _stakedFeeAmount) = splitFees(
                feeAmount,
                _liquidity,
                _stakedLiquidity
            );
