...
import {Math} from "openzeppelin-contracts-v3.4.2/contracts/math/Math.sol";
...
contract SugarHelper is ISugarHelper {
    ...
    function getSqrtRatioAtTick(
        int24 tick
    ) external pure override returns (uint160 sqrtRatioX96) {
    ...
    function getPopulatedTicks(
        ...
        uint256 maxBitmaps = Math.min(
            MAX_BITMAPS,
            uint256(type(int16).max - startBitmapIndex) + 1
        );
        ...
        for (uint256 j = 0; j < maxBitmaps; j++) {
            ...
            for (uint256 i = 0; i < 256; i++) {
                if (bitmap & (1 << i) > 0) {
                    ...
                    ) = ICLPool(pool).ticks(populatedTick);

                    populatedTicks[--numberOfPopulatedTicks] = PopulatedTick({
                        tick: populatedTick,
                        sqrtRatioX96: TickMath.getSqrtRatioAtTick(
                            populatedTick
                        ),
